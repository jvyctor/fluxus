...
import { CalendarIcon, Search, Info, FileText, DollarSign, Building, User, Plus, Package, ArrowRight, ArrowLeft, Check, X, Edit, Trash2, Save } from 'lucide-react';
...
export default function CadastroDID() {
  ...
  const [currentItemStep, setCurrentItemStep] = useState(1);
  ...
  const nextItemStep = () => {
    if (currentItemStep < 3) setCurrentItemStep(currentItemStep + 1);
  };

  const prevItemStep = () => {
    if (currentItemStep > 1) setCurrentItemStep(currentItemStep - 1);
  };
  ...
  const handleSaveDID = () => {
    ...
    if (!formData.processo || !formData.unidadeOrcamentaria || !formData.fornecedor || !formData.dotacao || !formData.descricao) {
      alert('Por favor, preencha todos os campos obrigatórios antes de continuar.');
      return;
    }
    
    // Avança para o step de inclusão de itens
    setMainStep(2);
  };

  const handleFinalizeDID = () => {
    if (items.length === 0) {
      alert('É necessário adicionar pelo menos um item ao DID.');
      return;
    }
    
    // Aqui você pode implementar a lógica para salvar o DID completo
    alert('DID salvo com sucesso!');
  ...
  const MainStepIndicator = ({ step, currentStep }: { step: number; currentStep: number }) => {
  ...
  const ItemStepIndicator = ({ step, currentStep }: { step: number; currentStep: number }) => {
    const isActive = step === currentStep;
    const isCompleted = step < currentStep;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Header da página com indicador de steps */}
        <div className="mb-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="pt-4">
                        <Button 
                          onClick={() => setShowItemsModal(true)}
                          variant="outline"
                          className="w-full h-12 border-blue-300 text-blue-600 hover:bg-blue-50 hover:border-blue-400"
                        >
                          <Plus className="mr-2 h-4 w-4" />
        ...
        {/* Ações baseadas no step atual */}
        <div className="mt-8 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0 sm:space-x-4">
          ...
          <div className="flex space-x-4">
            ...
            {mainStep === 1 ? (
              <Button 
                ...
                className="h-12 px-8 bg-blue-600 hover:bg-blue-700 shadow-lg hover:shadow-xl"
              >
              ...
              <Button 
                onClick={handleFinalizeDID}
                className="h-12 px-8 bg-green-600 hover:bg-green-700 shadow-lg hover:shadow-xl"
              >
              ...
              <div className="flex space-x-3">
                ...
                {currentItemStep < 3 ? (
                  <Button
                    ...
                    disabled={
                      (currentItemStep === 1 && (!currentItem.codigo || !currentItem.descricao)) ||
                      (currentItemStep === 2 && (!currentItem.quantidade || !currentItem.valorUnitario))
                    }
                    className="h-10 px-6"
                  >
                    Próximo
